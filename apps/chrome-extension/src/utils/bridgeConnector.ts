      ...
      while (true) {
        ...
        try {
          ...
          if (e?.message === 'Connection denied by user') {
            ...
            // Only log warning if we were previously connected (not initial connection attempts)
            if (wasConnected) {
              console.warn('Bridge connection lost, retrying...', e?.message || e);
            } else {
              // Silent retry for initial connection - this is expected when no CLI server is running
              console.log('[Bridge] Waiting for CLI connection... (no server found)');
            }
          }
